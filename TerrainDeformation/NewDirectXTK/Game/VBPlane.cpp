...
void VBPlane::toggleSnow(bool onOrOff, float valuePerTick)
{
	snowing = onOrOff;
...
void VBPlane::calculateSnowfall()
{
	...
	for (int i = -(m_width - 1) / 2; i<(m_width - 1) / 2; i++)
	{
		for (int j = -(m_height - 1) / 2; j<(m_height - 1) / 2; j++)
		{
			m_vertices[vert].Color += Color(0.0001f, 0.0001f, 0.0001f, 0.0f);
			m_vertices[vert++].Pos += Vector3(0, snowRate, 0);
			m_vertices[vert].Color += Color(0.0001f, 0.0001f, 0.0001f, 0.0f);
			m_vertices[vert++].Pos += Vector3(0, snowRate, 0);
			m_vertices[vert].Color += Color(0.0001f, 0.0001f, 0.0001f, 0.0f);
			m_vertices[vert++].Pos += Vector3(0, snowRate, 0);
